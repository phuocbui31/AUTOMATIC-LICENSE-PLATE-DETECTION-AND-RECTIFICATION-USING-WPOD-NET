...
class DataGenerator(object):
	...
	def _insert_data(self,x,y, pts):
		...
		if self._count < self._pool_size:
			...
			self._pts[self._count] = np.array(pts)
			self._count += 1
		else:
			idx = randint(0,self._pool_size-1)
			self._X[idx] = x
			self._Y[idx] = y
			self._pts[idx] = np.array(pts)

		self._sem.release()

	def _run(self):
		while True:
			x,y, pts = self._compute_sample()
			self._insert_data(x,y, pts)
			if self._stop:
				break
